...
The LinkML schema should stay focused on the domain objects. It should not model SAB internals such as users, jobs, audit events, change-set state, derived database values, or authorization. Those are application concerns.
...
The main `annotation` table should represent the current state of each annotation. Stable values used for querying or state management should be first-class columns: annotation ID, `db_object_id`, `ontology_class_id`, `evidence_type`, `assigned_by`, status, current version, created and updated timestamps, deletion state, record origin, and the source import job ID when applicable. Record origin is restricted to `direct` and `import`. Direct annotations must not reference an import job, while imported annotations must reference one. Indexes should support established lookup, filtering, and ordering patterns; not every first-class column requires its own index. More complex or multivalued structures can use child tables where they are important for query or export behavior.

Each mutation should create an immutable annotation version. A lightweight `annotation_version` table should store the full LinkML-shaped annotation snapshot, version number, actor, timestamp, and the source of the change. The main `annotation` table stores the current version and searchable annotation fields; historical versions are stored separately in `annotation_version`.
...
annotation_duplicate_reference
  current duplicate lookup data with annotation ID, duplicate base signature, and
...
Pagination should be used for "list" endpoints. The initial annotation list API should support structured filtering across the modeled Standard Annotation fields, except annotation extensions and annotation properties. Scalar fields should be stored directly in indexed PostgreSQL columns. Values from multivalued filterable fields can be stored individually in child tables so that filtering does not depend on JSONB scans.
